...
public class Game implements Runnable, KeyListener {
	...
	private final static int SCORE_BIG_ASTEROID = 70; // 70 points for killing a big asteroid
	private final static int SCORE_MEDIUM_ASTEROID = 50;
	private final static int SCORE_SMALL_ASTEROID = 30;
	private final static int SCORE_NUISSANCE = 20;
	private final static int SCORE_GHOST = 80;
	private final static int SCORE_HEAT_SEEKING_MISSILE = 50;
	...
	private static final int SPAWN_NEW_SHIP_FLOATER = 1000;
	private static final int SPAWN_NUISSANCE = 800;
	private static final int SPAWN_GHOST = 600;
	private static final int SPAWN_SHIELD_FLOATER = 500;
			...
			for (Movable movFoe : CommandCenter.getInstance().getMovFoes()) {
				...
				if (pntFriendCenter.distance(pntFoeCenter) < (nFriendRadiux + nFoeRadiux)) {
					...
					if ((movFriend instanceof Falcon) ){
						// if the falcon has a shield in position, the shield will be pierced
						if (CommandCenter.getInstance().getFalcon().getShielded()) {
							CommandCenter.getInstance().getFalcon().pierceShield();
						}
						// otherwise, if the falcon is not protected, then it will be removed
						else if (!CommandCenter.getInstance().getFalcon().getProtected()){
							CommandCenter.getInstance().getOpsList().enqueue(movFriend, CollisionOp.Operation.REMOVE);
							CommandCenter.getInstance().spawnFalcon(false); // not the beginning of game, so false
						}
					}
					//not the falcon
					else {
			...
			for (Movable movFloater : CommandCenter.getInstance().getMovFloaters()) {
				...
				if (pntFalCenter.distance(pntFloaterCenter) < (nFalRadiux + nFloaterRadiux)) {
					...
					if (movFloater instanceof NewShipFloater) {
						//when the falcon collides with a NewShipFloater, increment the number of falcons (lives)
						...
						//when the falcon collides with a ShieldFloater, increment the number of available shields
						CommandCenter.getInstance().getFalcon().addShield();
					}
	...
	private void killFoe(Movable movFoe) {
		...
		if (movFoe instanceof Asteroid){
			...
			if (astExploded.getSize() == 0) {
				...
				CommandCenter.getInstance().getOpsList().enqueue(new Asteroid(astExploded), CollisionOp.Operation.ADD);
				// add a corresponding score
				CommandCenter.getInstance().setScore(CommandCenter.getInstance().getScore() + SCORE_BIG_ASTEROID);
			} 
			//medium size asteroid exploded
				...
				CommandCenter.getInstance().getOpsList().enqueue(new Asteroid(astExploded), CollisionOp.Operation.ADD);
				// add score
				CommandCenter.getInstance().setScore(CommandCenter.getInstance().getScore() + SCORE_MEDIUM_ASTEROID);
			}
...
//				CommandCenter.getInstance().getOpsList().enqueue(new Debris(astExploded), CollisionOp.Operation.ADD);
				...
				// I decided to use Professor's version:
				createDebris(astExploded);
				// add score
				CommandCenter.getInstance().setScore(CommandCenter.getInstance().getScore() + SCORE_SMALL_ASTEROID);
			}
		}
		else if (movFoe instanceof Nuissance) {
			// add score
			CommandCenter.getInstance().setScore(CommandCenter.getInstance().getScore() + SCORE_NUISSANCE);
		}
		else if (movFoe instanceof Ghost) {
			CommandCenter.getInstance().setScore(CommandCenter.getInstance().getScore() + SCORE_GHOST);
		}
		else if (movFoe instanceof HeatSeekingMissile) {
			CommandCenter.getInstance().setScore(CommandCenter.getInstance().getScore() + SCORE_HEAT_SEEKING_MISSILE);
		}
	...
	private void createDebris(Movable movable) {
		if (movable instanceof Sprite){

			//cast the moveable to a Sprite
			Sprite sprite = (Sprite)movable;
			//get an array of points
			Point[] points =  sprite.getObjectPoints();
			//iterate all except the last segment to deconstruct this object
			for (int nC = 0; nC < points.length -1; nC++) {
				CommandCenter.getInstance().getOpsList()
						.enqueue(new Debris(sprite,points[nC], points[nC+1] ), CollisionOp.Operation.ADD);
			}
			//add the last segment
			Point pntFirst = points[0];
			Point pntLast = points[points.length -1];

			CommandCenter.getInstance().getOpsList()
					.enqueue(new Debris(sprite,pntFirst, pntLast ), CollisionOp.Operation.ADD);
	...
	private void spawnNuissance() {
		//make the appearance of nuissance which follows the falcon
		//the higher the level, the more frequent the appearance
		if (nTick % (SPAWN_NUISSANCE - nLevel * 7) == 0) {
			CommandCenter.getInstance().getOpsList().enqueue(new Nuissance(CommandCenter.getInstance().getFalcon()),
					CollisionOp.Operation.ADD);
		}
	}

	private void spawnGhost() {
		if (nTick % (SPAWN_GHOST - nLevel * 7) == 0) {
			CommandCenter.getInstance().getOpsList().enqueue(new Ghost(), CollisionOp.Operation.ADD);
		}
	}
